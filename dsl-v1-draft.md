...
6. [Восемь типов шагов](#6-восемь-типов-шагов)
   ...
   - [6.8. CallScenario (design-time only)](#68-callscenario-design-time-only)
...
сервере и на которое ссылаются `CallScenario.view.scenarioGuid` из родительских
сценариев. Это даёт:

- **корректный экспорт/импорт между средами** (dev→staging→prod): сценарий
  переезжает с тем же guid, ссылки `CallScenario` не ломаются;
...
### П9. Sub-scenario — дизайн-тайм артефакт, runner не знает

Шаг с `type: "CallScenario"` существует **только** в `scenarios.script`
(дизайн-тайм). При создании Incident'а **сервер inline'ит** его — то есть
заменяет на полный набор шагов дочернего сценария (с префиксацией id для
уникальности). В `incidents.scenario` остаётся **плоский граф**, и runner
никогда не видит `CallScenario`.

Это даёт три свойства:
- runner Web/iOS/Android **не реализуют** sub-scenario логику — никаких
  «прыжков», стека вложенности, особого `scenarioResult`;
- старые Incident'ы не ломаются при правке дочернего сценария — у них свой
  замороженный снапшот в `incidents.scenario`;
- JSONLogic в родителе после inline'инга может ссылаться на ответы
  дочерних шагов через стандартные `var` (с префиксом).
...
| Дочерний сценарий (ссылка) | `step.type = "CallScenario"` | резолвится сервером в `incidents.scenario` | **отсутствует** |
| Префикс id при inline | `step.view.stepIdPrefix` (или `step.id` по умолчанию) | внутри `incidents.scenario` | **отсутствует** |
...
| `type` | string enum | дискриминатор: `RadioButton` / `Checkbox` / `Select` / `Comment` / `Image` / `Datetime` / `Button` / `CallScenario` (последний — design-time, см. §6.8 и принцип П9) |
...
## 6. Восемь типов шагов

Семь интерактивных типов (6.1–6.7) **полностью покрывают функционал legacy
`IM/`** — оператор реально работает только с ними. Восьмой тип
(`CallScenario`, §6.8) — это **design-time артефакт**, который сервер
разворачивает в обычные шаги при создании Incident'а; runner про него не
знает (см. принцип П9).
...
### 6.8. CallScenario (design-time only)

**Дочерний (вложенный) сценарий.** Шаг-ссылка на другой сценарий, который
сервер **разворачивает (inline'ит)** в обычные шаги при создании Incident'а.

**Ключевая идея:** sub-scenario — это инструмент **переиспользования** в
дизайн-тайме. После inline-резолва получается обычный плоский граф, без
«стека вызовов», без особого runtime, без специальной поддержки на iOS/Android.

```jsonc
{
  "id": "do_followup",
  "type": "CallScenario",
  "title": "Связь с дежурным",
  "view": {
    "scenarioGuid": "11111111-2222-3333-4444-555555555555",
    "version": 5
  },
  "transitions": {
    "default": { "goto": "btn_close" }
  }
}
```
...
| Поле | Тип | Обязательное | Описание |
| --- | --- | --- | --- |
| `scenarioGuid` | uuid | ✓ | Идентификатор логического дочернего сценария (`scenario_definitions.guid` на сервере) |
| `version` | int ≥1 | ✓ | **Pin к конкретной версии**. Latest-ссылки в v1 не поддерживаются — см. зафиксированное решение Р4 в §13.1 |
| `stepIdPrefix` | string `^[a-z][a-z0-9_]{0,32}$` | — | Префикс для inline'инга id дочерних шагов. **Если опущен — сервер использует `step.id` как префикс автоматически.** Явный префикс нужен в редких случаях (см. ниже) |

#### Что делает сервер при создании Incident'а (inline-резолв)

Дано: родительский `script` содержит шаг
`{ id: "do_followup", type: "CallScenario", view: { scenarioGuid: X, version: 5 }, transitions: { default: { goto: "btn_close" } } }`.

Дочерний сценарий X (версия 5) состоит из шагов `[ask_name, choose_phone, final_check]`, начинается с `ask_name`, заканчивается на `final_check.transitions.default.goto = null` (см. ниже про «терминальный шаг»).

**Алгоритм:**

1. Прочитать дочерний `scenarios WHERE guid=X AND version=5`.
2. Сгенерировать префикс: `prefix = stepIdPrefix ?? parentStep.id` (= `"do_followup"` в нашем примере).
3. Для каждого шага дочернего сценария создать «новый» шаг с id `prefix + "__" + childStep.id` и **рекурсивно** обновить все ссылки `goto`/`when` (`var: state.X.value` → `var: state.do_followup__X.value`).
4. **Точка входа** (`initialStepId` дочернего → `prefix__ask_name`) — заменить ею `goto: "do_followup"` во всех родительских переходах.
5. **Точка возврата** — все «терминальные» шаги дочернего (где `transitions.default.goto = null` или `goto` ведёт на несуществующий шаг) перенаправить на то, куда указывал `do_followup.transitions.default.goto` в родителе (= `"btn_close"`).
6. Удалить шаг `do_followup` из набора шагов; результат записать в `incidents.scenario`.

После этого `incidents.scenario` — обычный плоский набор шагов:
```text
[ confirm_event, do_followup__ask_name, do_followup__choose_phone, do_followup__final_check, btn_close ]
```

Runner ничего не знает о sub-scenario.

#### Ссылки на ответы дочерних шагов в родительском JSONLogic

После inline'инга **все** id шагов уже плоские, поэтому `var` пишется так
же, как для собственных шагов:

```jsonc
"transitions": {
  "rules": [
    { "when": { "==": [ {"var": "state.do_followup__final_check.value"}, "ok" ] },
      "goto": "btn_close" }
  ],
  "default": { "goto": "escalate_path" }
}
```

В **дизайн-тайме** редактор должен подсказывать пользователю доступные пути
с префиксом — иначе пользователь не узнает имена шагов чужого сценария.
Это задача редактора, не схемы.

#### Передача данных от родителя к дочернему

В v1 — **никакой формальной передачи**. После inline'инга дочерние шаги
имеют доступ ко **всему** `state` через JSONLogic. Это работает, потому что
после inline-резолва нет «изоляции» — это один плоский граф (см. зафиксированное
решение Р4 в §13.1, аргумент Р3).

В дочернем сценарии можно использовать `{"var": "state.confirm_event.value"}`
напрямую — если шаг `confirm_event` пришёл из родителя, JSONLogic его
найдёт после inline.

> **Внимание дизайнера:** дочерний сценарий, который ссылается на конкретные
> `state.X.value` родителя, становится **неавтономным** — его нельзя
> запустить отдельно. Если важна автономность — пишите дочерние сценарии
> без зависимостей от внешнего state. Чёткой проверки на это в v1 нет (это
> ответственность пользователя).

#### Терминальный шаг дочернего сценария

Чтобы сервер мог сшить «выход» из дочернего сценария с родителем, дочерний
должен иметь хотя бы один **терминальный** шаг. Терминальный = «не имеет
исходящего перехода в другой шаг»:

- `transitions.default.goto: null`, или
- `transitions.default` содержит только `actions` без `goto`, или
- `transitions.default.goto` ссылается на несуществующий step.id (это нормально
  для самостоятельного сценария, но при inline-резолве будет сшито с
  родителем).

Если терминальных шагов несколько — все они после inline сшиваются в один
и тот же родительский `goto`.

**Если в дочернем последний шаг имеет `actions: [{"type": "finish"}]`** —
после inline-резолва этот `finish` **не выполняется**, потому что переход
сшивается на родительский `goto`, и actions терминального шага заменяются
на actions родительского `do_followup.transitions.default.actions`. Это
важно: дочерний сценарий, спроектированный «как самостоятельный»,
автоматически адаптируется при включении в родителя.

#### Когда `stepIdPrefix` обязателен

Только в крайне редком случае: один и тот же дочерний сценарий вставлен
**дважды** в одного родителя. По умолчанию префикс = `step.id`, и поскольку
`step.id` **уникален в рамках родителя**, конфликта нет:

```jsonc
{ "id": "followup_morning", "type": "CallScenario", "view": { "scenarioGuid": X, "version": 5 } },
{ "id": "followup_evening", "type": "CallScenario", "view": { "scenarioGuid": X, "version": 5 } }
```

Префиксы получатся `followup_morning__*` и `followup_evening__*`, они уникальны.

Явный `stepIdPrefix` имеет смысл задавать только если хотите **более
короткое** имя для inline-шагов в `scenarioResult.history` (например,
`step.id = "do_followup_call_to_dispatcher_with_priority_check"` и
`stepIdPrefix = "fc"`).

#### Запрет циклов

Цикл — это когда сценарий A ссылается (через цепочку `CallScenario`) сам
на себя. Сервер при сохранении сценария обязан проверить отсутствие циклов
DFS-обходом по графу `CallScenario`-шагов. При обнаружении цикла —
422 Unprocessable Entity.

Это решение Р6 (см. §13.1).

#### Маппинг с legacy `IM/`

В legacy `IM/` концепции вложенных сценариев **не было** (анализ см.
`server-analysis.md`). `CallScenario` — это **новая** возможность, не
переезд legacy-фичи. Никаких сценариев из `IM/` для миграции
конвертировать в `CallScenario` не нужно — все они переезжают плоско.
...
- `CallScenario.scenarioGuid` ссылается на несуществующий или архивированный
  дочерний сценарий;
- `CallScenario.version` указывает на несуществующую версию;
- **цикл `CallScenario`-зависимостей** (A → B → A) — обнаруживается DFS-обходом
  при сохранении (решение Р6, см. §13.1).
...
| **Latest-ссылки в `CallScenario`** (без `version`) | автоматический подхват правок дочернего во всех родителях | сейчас `version` обязателен (решение Р4, см. §13.1) |
| **Передача параметров (`inputs`/`outputs`) в `CallScenario`** | если sub-scenario должен быть полностью автономным | сейчас доступ к `state` через JSONLogic после inline-резолва (без явного контракта) |
...
#### Р4. Sub-scenario: гибрид «дизайн-тайм ссылка + inline-резолв при создании Incident»

**Решение:** шаг `CallScenario` — это design-time артефакт. Сервер при
создании Incident'а **разворачивает** ссылку в полный набор шагов
дочернего сценария (с префиксацией id), складывает в `incidents.scenario`
плоский граф. Runner про `CallScenario` не знает.

**Контекст:** альтернативы — чистый inline (copy-paste при сохранении в
редакторе) и чистый reference (runner подгружает дочерний динамически).

**Почему гибрид:**
- Один источник истины для дочернего сценария (правка дочернего → видна
  во всех новых Incident'ах автоматически, без перезаписи родителей).
- Снапшот в `incidents.scenario` остаётся целостным и плоским — старые
  Incident'ы не ломаются при правке дочернего, runner не нуждается в
  доступе к нескольким сценариям.
- Runner Web/iOS/Android **не реализуют** sub-scenario логику. Это
  главная архитектурная победа: 3 платформы остаются простыми.
- Соответствует принципу П9.

**Также зафиксировано в этом решении:**
- **Pin к версии** обязателен (`view.version`). Latest-ссылки в v1 не
  поддерживаются — предсказуемость > удобство автоапдейта.
- **Передача данных** между родителем и дочерним — через общий `state`
  после inline'инга, без формальных `inputs`/`outputs`. Это работает,
  потому что после inline нет изоляции.
- **Префикс id** дочерних шагов = `step.id` родительского `CallScenario`
  по умолчанию; явный `stepIdPrefix` опционален.

**Когда пересматриваем:**
- Понадобится автоматический подхват правок дочернего во всех родителях →
  ввести latest-ссылки (через дополнительное поле, обратно совместимо).
- Понадобится полная автономность дочерних сценариев (запуск отдельно с
  передачей параметров) → ввести `inputs`/`outputs`-контракт.

#### Р5. Запрет циклов `CallScenario`-зависимостей

**Решение:** сервер при сохранении сценария **обязан** обнаружить циклы
A → B → A в графе ссылок `CallScenario` через DFS-обход. При обнаружении —
422 Unprocessable Entity с понятным сообщением.

**Контекст:** альтернативы — лимит глубины (например, 10 уровней) и
allow-with-warning. Цикл при inline-резолве даёт бесконечную рекурсию —
проверка обязательна в любом случае.

**Почему запрет, а не лимит:**
- Цикл в иерархии sub-scenario — **практически всегда ошибка**. Легитимных
  кейсов нет.
- Лимит даёт ложное ощущение «работает» при глубине 9, и неожиданный
  отказ при 10. Запрет сразу понятен.
- Альтернатива «allow-with-warning» — это ловушка, потому что warning при
  сохранении сценария можно игнорировать, а реальная проблема всплывёт
  только при создании Incident'а в production.

**Когда пересматриваем:** не предполагается. Если потребуется рекурсия —
вводится отдельный механизм (например, BPMN-style sub-process loop).
...
| (новое) Переиспользование сценариев | реализовано через `CallScenario` (§6.8); inline-резолв на сервере (П9, Р4) |
