...
- **Что это:** декларативный JSON-формат, описывающий **граф шагов** UI-сценария
  (7 типов элементов управления) с условными переходами на JSONLogic.
...
- **Главные архитектурные обещания:**
  ...
  3. **JSONLogic для условий** — стандартный whitelist 16 операторов, готовые
     парсеры для всех платформ.
- **Как валидируется:** двумя уровнями — JSON Schema (структура) + сервер
  (семантика: висящие goto, дубли id).
...
| --- | --- | --- |
| [`dsl-v1-draft.md`](./dsl-v1-draft.md) | Главная **спецификация** DSL: принципы, типы шагов, JSONLogic-whitelist, два уровня валидации, зафиксированные решения | ~1600 строк |
...
| [`proposal-versioning.md`](./proposal-versioning.md) | **Предложение для команды сервера**: миграция 005, версионирование `scenarios` | ~780 строк |
...
6. **`dsl-v1-draft.md` §1 «Принципы»** — принципы, на которых всё держится. Особенно П2 и П5.
...
2. **`dsl-v1-draft.md`** — целиком. Основные точки внимания:
   - §1 принципы и §13.1 «Зафиксированные решения»;
   - §6 типы шагов;
   - §7 JSONLogic whitelist и §11 уровни валидации.
3. **`server-analysis.md`** §5 «Расхождения» — места, где предложение и текущий сервер разъезжаются, и как это лечится.
4. **`examples/architecture/A5-anti-patterns/`** + README — где границы валидации (что ловит схема, что — сервер, а что — только code review).
5. **`proposal-versioning.md`** §6 — изменения в Go-бэкенде, чтобы это всё работало.
...
4. **`dsl-v1-draft.md`** §11 — детали семантической валидации.
5. **`examples/architecture/A5-anti-patterns/bad-1` и `bad-2`** — что ваша семантическая валидация **обязана** отвергать с 422.

### Если вы разработчик runner'а (Web/iOS/Android) (~2 часа)

1. **`dsl-v1-draft.md`** §1 принципы (особенно П1, П3, П8) и §6 типы шагов — это всё, что вы рендерите.
