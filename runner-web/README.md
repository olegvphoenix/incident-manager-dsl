...
- **CallScenario не запускается.** Сценарии, содержащие `CallScenario`,
  показываются в списке отдельной секцией с пометкой «требует серверного
  inline-резолва». Алгоритм есть в `../dsl-v1-draft.md` §6.8 — добавим клиентский
  resolver, если потребуется на демо.
